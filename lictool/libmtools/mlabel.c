...
int set_label(char *device, char *label);
...
int
set_label(char *device, char *label)
{
    char drive = 'J';
	direntry_t entry;
	Stream_t *RootDir;
	...
	read_config();
	init_clash_handling(&ch);
    strcpy(device_file, device);
	RootDir = open_root_dir(drive, O_RDWR, NULL);
	if(!RootDir) {
		fprintf(stderr, "Cannot initialize device %s\n", device);
		exit(1);
	}
	...
	if (r == -2) {
		FREE(&RootDir);
		fprintf(stderr, "Cannot lookup labels\n");
		exit(1);
	}
	...
	if (label[0] != '\0') {
		ch.ignore_entry = 1;
		result = mwrite_one(RootDir,label,0,labelit,NULL,&ch) ?
		  0 : 1;
	}
#endif

	have_boot = 0;
	if(label[0] || set_serial != SER_NONE) {
		Fs = GetFs(RootDir);
		have_boot = (force_read(Fs,boot.characters,0,sizeof(boot)) ==
			     sizeof(boot));
	}
	...
	if(label[0]){
        ...
        shrtLabel = label;
		cp = GET_DOSCONVERT(Fs);
		label_name_pc(cp, shrtLabel, 0, &mangled, &dosname);
