...
static bool  read_sgio(const char *devname, char *buf)
{
	int fd = -1, rc;
	bool status = false;
	unsigned char cdb[] = { 0x85, 0x08, 0x0e, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0xec, 0x00 };
...
bool probe_ide(const string & name,
hwNode & device)
{
  ...
  {
    	close(fd);
    	if (!read_sgio(device.getLogicalName().c_str(), (char *)pidentity)) {
	    return false;
	}
	for (int i = 0; i < 256; i++) {
		unsigned val  = pidentity[i];
		pidentity[i] = ((val & 0xFF) << 8) | (val >> 8);
	}
