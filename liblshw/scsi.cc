...
static string getword(char *line)
{
	char *word;
...
static void extractinq(char *line, hwNode &n)
{
	char *label, *str;

	label = "Vendor identification: ";
	if (str = strstr(line, label)) {
		str += strlen(label);
		n.setVendor(getword(str));
	}
	label = "Product identification: ";
	if (str = strstr(line, label)) {
		str += strlen(label);
		n.setProduct(getword(str));
	}
	label = "Unit serial number: ";
	if (str = strstr(line, label)) {
		str += strlen(label);
		n.setSerial(getword(str));
	}
}
...
static void find_logicalname(hwNode & n)
{
  ...
  for (i = sg_map.begin(); i != sg_map.end(); i++)
  {
    if (i->second == n.getHandle())
    {
	  ...
	  /* Use sg_inq and get more inquiring info */
	  char cmd[512];
	  sprintf(cmd, "sg_inq %s", i->first.c_str());
	  FILE *fp = popen(cmd, "r");
	  char *line = NULL;
	  size_t len = 0;
	  ssize_t read;
...
static void scan_sg(hwNode & n)
{
  ...
  if(glob(SG_X, 0, NULL, &entries) == 0)
  {
    ...
    {
        char *numstr;
        numstr = strpbrk(entries.gl_pathv[j], "0123456789");
        if (!numstr)
                continue;

      sg = strtol(numstr, NULL, 10);
  ...
  {
    extern bool probe_ide(const string & name, hwNode & device);
    probe_ide("", device);
    //device.setDescription("ATAZ " + device.getDescription());
    device.setVendor("");
    goto add_it;
  }
  else
